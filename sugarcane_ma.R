...
m_obs <- rma.mv(yi = PCY_pct, V = vi, mods = ~ delta_T_C,
                 random = list(~ 1 | obs),
...
m_study <- rma.mv(yi = PCY_pct, V = vi, mods = ~ delta_T_C,
                random = list(~ 1 | study_id/obs, ~ 1 | crop_model),
                data = dat, method = "REML")
summary(m_study)
anova(m_base, m_study)

data.frame(model  = c("obs + crop", "obs + site + crop", "study/obs + crop"),
           AIC    = c(AIC(m_base), AIC(m_site), AIC(m_study)),
           BIC    = c(BIC(m_base), BIC(m_site), BIC(m_study)),
           logLik = c(logLik(m_base), logLik(m_site), logLik(m_study)),
           row.names = NULL)
confint(m_study, sigma2 = 1)


pdf("ma_regplot.pdf", width = 7, height = 6)
regplot(m_study, mod = "delta_T_C", psize = 1, xlab = "Temperature increase (°C)", ylab = "PCY (%)")
dev.off()
...
abline(a = coef(m_study)[1], b = coef(m_study)[2], col = "black", lwd = 3)
